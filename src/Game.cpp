...
Game::Game( Point screenResolution, std::map<ImagePrimitiveType, Point> imageSizesMap ) :
		...
		powerupGenerator_(	actorsContainer_, idGenerator, pythonModule_, drawingSystem_, gameConfiguration_, boxService_, box2dObjectsContainer_,
							  imageScalesContainer_, contactComponentsContainer_, rocketProxy_, projectilesGenerator_, inputManager_,
							  gameTimeProvider_, musicManager_, powerupCounter_, rocketLife_),
		randomPowerupsGenerator_( new RandomPowerupGenerator(rocketProxy_, gameConfiguration_, gameTimeProvider_, powerupGenerator_, powerupCounter_, randomNumbersProvider_)),
		commonTypesVisualizer_( new CommonTypesVisualizer(pythonModule_)),
        rocketProxy_( new RocketActorProxy( std::make_shared<Actor>(idGenerator.getActorId()))),
    ...
    createBothModesActors();
    createMultiplayerActors();
...
void Game::startSinglePlayerGame(int difficulty) {
    ...
    rocketProxy_->setNewRocketActor( std::make_shared<Actor>(idGenerator.getActorId()));
    createBothModesActors();
    actorsContainer_->OnStart();
    lifeIndicatorService_->createHearts();
}
...
void Game::createBothModesActors() {
    ...
    rocketProxy_->getRocketActor()->addComponent(std::make_shared<Box2dComponent>(boxService_, gameConfiguration_, box2dObjectsContainer_.getRocketObject()));
    rocketProxy_->getRocketActor()->addComponent(std::make_shared<PositionComponent>(pythonModule_));
    rocketProxy_->getRocketActor()->addComponent(std::make_shared<DrawingComponent>(boundariesDuplicationsDrawingSystem_ , ImagePrimitiveType::Rocket, imageScalesContainer_.getImageScale(ImagePrimitiveType::Rocket)));
    ...
    rocketProxy_->getRocketActor()->addComponent(rocketMovingComponent);
    rocketProxy_->getRocketActor()->addComponent( std::make_shared<PythonActorComponent>(pythonModule_));
    rocketProxy_->getRocketActor()->addComponent( std::make_shared<ActorTypeComponent>(ActorType_Rocket,  pythonModule_));
    rocketProxy_->getRocketActor()->addComponent( std::make_shared<PositionSettingComponent >(true, pythonModule_));
    rocketProxy_->getRocketActor()->addComponent( std::make_shared<ScreenBoundariesTeleportationComponent>(gameConfiguration_));
    rocketProxy_->getRocketActor()->addComponent( std::make_shared<RocketShootingComponent>(gameConfiguration_, projectilesGenerator_, inputManager_, gameTimeProvider_, musicManager_, Rotation(0)));
    rocketProxy_->getRocketActor()->addComponent( std::make_shared<RocketCollisionComponent>( contactComponentsContainer_, rocketLife_, gameConfiguration_, gameTimeProvider_, musicManager_));

    auto rocketTail = std::make_shared<Actor>(idGenerator.getActorId());
    rocketTail->addComponent(std::make_shared<RocketTailPositionComponent>(rocketProxy_->getRocketActor(), gameConfiguration_));
    ...
    actorsContainer_->addActor(rocketProxy_->getRocketActor());
    actorsContainer_->addActor(rocketTail);
    rocketMovingComponent->setRocketTail(rocketTail);
}

void Game::createMultiplayerActors() {
